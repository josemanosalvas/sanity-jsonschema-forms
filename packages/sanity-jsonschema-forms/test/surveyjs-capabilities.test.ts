...
describe('SurveyJS capabilities beyond JSON Schema (probe)', () => {
  test('pages: a survey is a sequence of pages with navigation state', () => {
    const m = new Model({pages: [{name: 'p1', elements: [{type: 'text', name: 'a'}]}, {name: 'p2', elements: [{type: 'text', name: 'b'}]}]})
  ...
  test('visibleIf: a question hides itself and drops out of validation', () => {
    const m = new Model({elements: [{type: 'boolean', name: 'other'}, {type: 'text', name: 'detail', isRequired: true, visibleIf: '{other} = true'}]})
    m.data = {other: false}
  ...
  test('enableIf and requiredIf: enablement and required-ness are expressions too', () => {
    const m = new Model({elements: [{type: 'text', name: 'a'}, {type: 'text', name: 'b', enableIf: '{a} notempty', requiredIf: '{a} = "x"'}]})
    m.data = {}
  ...
  test('calculatedValues: derived values exist in the data model, not in any question', () => {
    const m = new Model({calculatedValues: [{name: 'total', expression: '{qty} * {price}'}], elements: [{type: 'text', name: 'qty', inputType: 'number'}, {type: 'text', name: 'price', inputType: 'number'}]})
  ...
  test('expression validators: cross-field rules in a runtime expression language', () => {
    const m = new Model({elements: [{type: 'text', name: 'start', inputType: 'number'}, {type: 'text', name: 'end', inputType: 'number', validators: [{type: 'expression', expression: '{end} > {start}', text: 'End must follow start.'}]}]})
  ...
  test('paneldynamic: repeating groups with add/remove and per-panel validation', () => {
    const m = new Model({elements: [{type: 'paneldynamic', name: 'people', minPanelCount: 1, templateElements: [{type: 'text', name: 'name', isRequired: true}]}]})
    m.data = {people: [{name: ''}]}
    expect(m.validate()).toBe(false)
